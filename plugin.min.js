...
tinymce.PluginManager.add('autocite', function(editor) {
	...
	editor.on('NodeChange', function(e){
		...
		if (
			...
			(($(e.element).find('[data-cite-type]').length > 0) && ($(e.element).find('[data-cite-type]').parent()[0] == e.element) && ($(e.element).find('[data-cite-type]').parent()[0].nodeName == 'SPAN'))
		){
			...
			if (matches){//if prev node 'text' and 'id' is changed then generate 'id' with the current text and replace it if any change
                //if(changedNodeText != originalText){
					if (($(originalElement).attr('data-cite-type') != undefined) && (/; /.test(originalText))){
						originalTextArray = originalText.split('; ');
						$(originalTextArray).each(function(i, citationText){
							if (i != 0){
								$(originalElement).before('; ');
							}
							var getRIDNode = $('<span data-replicate="true" class="'+$(originalElement).attr('class')+'">'+citationText+'</span>');
							commonjs.general.updateCitationData(getRIDNode, false);
							$(getRIDNode).insertBefore(originalElement);
						});
						$(originalElement).remove();
						originalElement = $('[data-replicate]').first().attr('data-cite-type', 'insert');
						$('[data-replicate]').removeAttr('data-replicate');
					}
					...
					if (citeClass[0]){//tag it as citation only response id true
						...
						if (((getRIDNode.attr('data-citation-string') != undefined) && ($(originalElement).attr('data-citation-string').replace(/^\s|\s$/g, '') != getRIDNode.attr('data-citation-string').replace(/^\s|\s$/g, ''))) || ($(originalElement).attr('data-cite-type') != undefined)){
							...
							$(originalElement).removeAttr('data-cite-type').removeAttr('data-auto-cite');
							$('.autoCiteSuggestion').remove();
						...
						if (originalText == ''){
							$(originalElement).attr('data-cite-type', 'delete');
							triggerReorder();
						}else{
		...
		if (($(e.element).find('[class*="Ref"]').length > 0) && ($(e.element).find('[class*="Ref"]').text() == $(e.element).text().replace(/^[\s\u00A0]|[\s\u00A0]$/, ''))){
			changedNode = $(e.element).find('[class*="Ref"]')[0];//assigning the current node as changed node
		}
	...
	function triggerReorder(){
		...
		var insertNew = false;
		if ($('[data-cite-type="insert"]').length > 0){
		...
		if (insertNew){
			...
			(new PNotify({title: 'Confirmation Needed',text: 'You have cited a '+object+' which is not found in this article, Do you need to insert it as a new one?',icon: 'glyphicon glyphicon-question-sign',hide: false,confirm: {confirm: true},buttons: {closer: false,sticker: false},history: {history: false},
			stack: false,
	...
	function parseCurrentLine(editor, end_offset, delimiter, enter, key) {
		...
		if (autoCiteTriggered){
			while ((! /[\s\u00A0\(\[](Fig|Vid|Tab|ima|sch|chart|box|Supplement|Equation)/gi.test(rng.toString())) && (end - 1) >= 0) {
